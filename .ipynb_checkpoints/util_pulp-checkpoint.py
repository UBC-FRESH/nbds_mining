...
def cmp_c_cflw(fm, path, expr, mask=None):
    """
...
def cmp_c_ci(fm, path, yname, mask=None):
    """
...
def gen_scenario(fm, name='base', util=0.85, harvest_acode='harvest',
    ...
    if obj_mode == 'max_hv':  # maximize harvest volume
        sense = ws3.opt_pulp.SENSE_MAXIMIZE
        zexpr = vexpr
    elif obj_mode == 'min_ha':  # minimize harvest area
        sense = ws3.opt_pulp.SENSE_MINIMIZE
        zexpr = '1.'
    else:
...
def run_scenario(fm, scenario_name='base'):
    ...
    cflw_ha = {p: 0.05 for p in fm.periods}
    cflw_hv = {p: 0.05 for p in fm.periods}
